...
#import "TPSecurityViewController.h"
#import "MyUtils.h"

@interface TPSecurityViewController ()
...
@property (strong, nonatomic) IBOutlet UITextField *mConfirmTextField;

@end

@implementation TPSecurityViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    if ([MyUtils shared].tempUser.email) {
        self.mUserNameTextField.text = [MyUtils shared].tempUser.email;
        [MyUtils shared].tempUser.userName = [MyUtils shared].tempUser.email;
    }
    
    if ([MyUtils shared].tempUser.password) {
        self.mPasswordTextField.text = [MyUtils shared].tempUser.password;
        self.mConfirmTextField.text = [MyUtils shared].tempUser.password;
    }
...
-(void) movePanel:(BOOL) isAnimated
{
    ...
    if (isAnimated) {
        ...
        self.mInputPanelTopSpacingConstant.constant = -self.mFeaturedImageView.frame.size.height;//-keyboardSize.height;
        ...
        self.mBottomViewBottomSpaceConstant.constant = -keyboardSize.height;
        self.mInputPanelTopSpacingConstant.constant = -keyboardSize.height;
...
- (IBAction)onOk:(id)sender {
    ...
    if (self.mUserNameTextField.text.length == 0 || self.mPasswordTextField.text.length == 0 || (![self.mPasswordTextField.text isEqualToString:self.mConfirmTextField.text])) {
        return;
    }
    
    [MyUtils shared].tempUser.userName = self.mUserNameTextField.text;
    [MyUtils shared].tempUser.password = self.mPasswordTextField.text;
    ...
    if (self.isPanelMoved) {
        [self moveBackPanel:YES Completion:^{
            if ([MyUtils shared].tempUser.accountType == TPAccountType_TRAVELER) {
                ...
                [self performSegueWithIdentifier:@"ViewPayment" sender:self];
            }
...
- (IBAction)unwindToSecurity:(UIStoryboardSegue *)unwindSegue
{
}
/*
