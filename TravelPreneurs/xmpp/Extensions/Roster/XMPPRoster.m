...
- (void)addUser:(XMPPJID *)jid withNickname:(NSString *)optionalName groups:(NSArray *)groups subscribeToPresence:(BOOL)subscribe{
	...
	if(subscribe)
	{
		[self subscribePresenceToUser:jid groupID:optionalName];
	}
}
...
- (void)subscribePresenceToUser:(XMPPJID *)jid groupID:(NSString *)groupID {
    // This is a public method, so it may be invoked on any thread/queue.
	...
	XMPPPresence *presence = [XMPPPresence presenceWithType:@"subscribe" to:[jid bareJID] groupID:groupID];
	[xmppStream sendElement:presence];
}
...
- (void)xmppStream:(XMPPStream *)sender didReceivePresence:(XMPPPresence *)presence
{
    ...
    {
        XMPPJID *userJID = [[presence from] bareJID];
        [self removeUser:userJID];
        [multicastDelegate removedUser:userJID];
    }
    else {
		#ifdef _XMPP_MUC_H
