...
+ (XMPPPresence *)presenceWithType:(NSString *)type to:(XMPPJID *)to groupID:(NSString *)groupID {
	return [[XMPPPresence alloc] initWithType:type to:to groupID:groupID];
}
...
- (id)initWithType:(NSString *)type to:(XMPPJID *)to groupID:(NSString *)groupID {
    if ((self = [super initWithName:@"presence"]))
	{
		if (type)
			[self addAttributeWithName:@"type" stringValue:type];
        ...
        if (groupID)
			[self addAttributeWithName:@"groupID" stringValue:groupID];
	}
	return self;
}
