...
- (void)setHostAddress:(NSString *)newHostAddress
{
    if (dispatch_get_specific(xmppQueueTag))
    {
        if (hostAddress != newHostAddress)
        {
            hostName = [newHostAddress copy];
        }
    }
    else
    {
        NSString *newHostAddressCopy = [newHostAddress copy];
        
        dispatch_async(xmppQueue, ^{
            hostAddress = newHostAddressCopy;
        });
...
- (BOOL)connectWithTimeout:(NSTimeInterval)timeout error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
        ...
        else
        {
            ...
            NSError *connectErr = nil;
            result = [self connectToHost:hostAddress onPort:hostPort withTimeout:XMPPStreamTimeoutNone error:&connectErr];
