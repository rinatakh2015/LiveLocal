...
- (void)layoutSubviews
{
    ...
    prevIsPortrat =  UIDeviceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation) ? 1 : 0;
}
...
- (void)scrollViewDidScroll:(__unused UIScrollView *)scrollView
{
    if (!_suppressScrollEvent)
    {
        ...
        int isPortrat = UIDeviceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation) ? 1 : 0;

        if (isPortrat == prevIsPortrat) {
